...
A missing file is normal. A malformed one throws rather than quietly running a
...
Every provider speaks the Anthropic Messages API, so one client covers all of
them and any other gateway that does. Point `provider.baseURL` and
`provider.model` at anything compatible.
...
Reasoning is not a cost to tolerate here — it is the thing doing the work. With
thinking disabled, a fast model blocked `rm -rf node_modules`, which is the most
common destructive-looking-but-routine command there is. With thinking on it was
correct, and then it sat in the same latency band as Spark anyway.
...
more tokens, and more time, so `reasoning` drives all three rather than being a
flag you set alone.

`maxTokens` is a floor rather than a preference. Spark spends 1,000 to 1,900
tokens thinking before it answers, and a budget that cuts in before it finishes
returns nothing at all — not a bad answer, an empty one. The shipped default is
3,000.
...
**Under Muse, the command is the only thing that works.** Muse runs hook commands
with a scrubbed environment, so no variable you export reaches the hook.
...
Each replaces the shipped file whole; the two are not merged. Most people change
`rulesPath` and never touch the other.
...
by name. A replacement framework must keep a `<rules>` line, or auto-mode refuses
to start rather than run a policy with no rules in it.
